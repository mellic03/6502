    ...
    SP(0xFD), PC(0x0000)
{
    ...
    mFtab[0x09] = Inst("ORA#oper   ", &X::InstrORA, &X::LoadIMM,  2);
    mFtab[0x05] = Inst("ORAoper    ", &X::InstrORA, &X::LoadZPG,  3);
    mFtab[0x15] = Inst("ORAoper,X  ", &X::InstrORA, &X::LoadZPGX, 4);
    mFtab[0x0D] = Inst("ORAoper    ", &X::InstrORA, &X::LoadABS,  4);
    mFtab[0x1D] = Inst("ORAoper,X  ", &X::InstrORA, &X::LoadABSX, 4);
    mFtab[0x19] = Inst("ORAoper,Y  ", &X::InstrORA, &X::LoadABSY, 4);
    mFtab[0x01] = Inst("ORA(oper,X)", &X::InstrORA, &X::LoadINDX, 6);
    mFtab[0x11] = Inst("ORA(oper),Y", &X::InstrORA, &X::LoadINDY, 5);
