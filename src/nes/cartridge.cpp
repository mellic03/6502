...
NesEmu::Cartridge::Cartridge( const std::string &path )
{
    ...
    size_t mDataSz = stream.tellg();

    stream.seekg(0, std::ifstream::beg);
    mRaw = std::unique_ptr<uint8_t[]>(new uint8_t[mDataSz]);
    stream.read(reinterpret_cast<char*>(mRaw.get()), mDataSz);


    mFormat = _readfmt(mRaw.get());
    mFile->as_iNES = NesFile::iNES(mRaw.get());
